...
mod tests {
    ...
    fn test_price_max_price() {
        ...
        */
        ...
        // chopped off 4 decimals from maxPrice of 2960410000000000
        let price_eth = Decimal::try_from_price(296041000000, 8, 8, 4).unwrap();
        // precision 4
        assert_eq!(price_eth.value, 29604100);
        // 8 (20-4-8)
        assert_eq!(price_eth.decimal_multiplier, 8);
        // 12 decimals (4+8)
        assert_eq!(price_eth.to_unit_price(), 2960410000000000);
        // chopped off 2 decimals from maxPrice
        let price_btc = Decimal::try_from_price(7695092578398765000000000, 20, 8, 2).unwrap();
        // precision 2
        assert_eq!(price_btc.value, 7695092);
        // 10 (20-2-8)
        assert_eq!(price_btc.decimal_multiplier, 10);
        // 12 decimals (2+10)
        assert_eq!(price_btc.to_unit_price(), 76950920000000000);
        // chopped off 4 decimals from maxPrice of 1000000000000000000000000
        let price_usdc = Decimal::try_from_price(100000000000000000000, 20, 6, 6).unwrap();
        // precision 6
        assert_eq!(price_usdc.value, 1000000);
        // 8 (20-6-6)
        assert_eq!(price_usdc.decimal_multiplier, 8);
        // 14 decimals (6+8)
        assert_eq!(price_usdc.to_unit_price(), 100000000000000);

        let fiat_value_eth = price_eth.to_unit_price() * 10u128.pow(8);
        // 20 decimals (unit price decimals+token_decimals = 20 for one full unit)
        assert_eq!(fiat_value_eth, 296041000000000000000000u128);
