...
use switchboard_on_demand_client::{SbContext, Gateway, PullFeed, oracle_job::OracleJob, SlotHashSysvar, BatchFeedRequest, FetchSignaturesBatchParams, CrossbarClient, Submission, SWITCHBOARD_ON_DEMAND_PROGRAM_ID};
use base64::prelude::*;
...
impl<'a, C: Clone + Deref<Target = impl Signer>> PostPullOraclePrices<'a, C> for SwitchboardPullOracle<'a, C> {
    async fn fetch_price_update_instructions(
        ...
        let payer = self.gmsol.payer().clone();
        let mut prices = HashMap::<Pubkey, Pubkey>::new();
        for (idx, submissions) in price_signatures.into_iter().enumerate() {
            let feed = feeds[idx].clone();
            prices.insert(feed.clone(), feed.clone());
            ...
            let mut submit_ix = Instruction {
                program_id: SWITCHBOARD_ON_DEMAND_PROGRAM_ID.clone(),
                data: ix_data.data(),
            ...
            let ix = self.switchboard.rpc().pre_instruction(submit_ix)
                .lookup_tables(luts.clone().into_iter().map(|x| {
                    (x.key, x.addresses.clone())
                }));
            ixns.try_push_post(ix)?;
        }
        Ok((ixns, HashMap::from([(PriceProviderKind::Switchboard, prices)])))
    }
}
